@extends('layouts.app')

<script src="https://admissions.uoel.edu.pk/admin_asset/js/formplugins/select2/select2.bundle.js"></script>
<script>
    $(document).ready(function() {
        $(function() {
            $('.select2').select2();

        });
    });
</script>


@section('page-name', 'Edit Address Details')
@section('admin-main')
    <div class="row">
        <div class="col-xl-12">
            <div id="panel-12" class="card">
                <div class="card-header">
                    <h2 class="card-title">
                        Edit Address Detail
                    </h2>
                </div>
                <div class="card-body show">
                    <div class="panel-content">
                        <form method="post" action="{{ route('admission.address-detail.update', $addressDetail->id) }}"
                            enctype="multipart/form-data">@csrf

                            <div class="row mb-3">
                                <label for="temp_postal_address" class="col-sm-2 col-form-label"> Postal Address: <span
                                        class="text-danger">*</span></label>
                                <div class="col-sm-8">
                                    <textarea class="form-control" name="temp_postal_address" id="temp_postal_address" value="jbd" required>{{ $addressDetail->temp_postal_address }}</textarea>
                                    <x-input-error class="mt-2 @error('temp_postal_address') has-error @enderror"
                                        :messages="$errors->get('temp_postal_address')" />
                                </div>
                            </div>
                            <div class="row mb-3">
                                <label for="temp_city" class="col-sm-2 col-form-label"> Postal City: <span
                                        class="text-danger">*</span></label>
                                <div class="col-sm-8">
                                    <select name="temp_city" id="temp_city" class="form-control form-select select2"
                                        required>
                                        <option value="">Choose Postal City</option>
                                        <option value="{{ $addressDetail->temp_city }}" selected>
                                            {{ $addressDetail->temp_city }}</option>
                                        <option value="Attock">Attock</option>
                                        <option value="Fatehjang">Fatehjang</option>
                                        <option value="Ghorgushti">Ghorgushti</option>
                                        <option value="Hasan Abdal">Hasan Abdal</option>
                                        <option value="Hazro">Hazro</option>
                                        <option value="Jand">Jand</option>
                                        <option value="Pindi Gheb">Pindi Gheb</option>
                                        <option value="Bahawalnagar">Bahawalnagar</option>
                                        <option value="Chishtian">Chishtian</option>
                                        <option value="Dunga Bunga">Dunga Bunga</option>
                                        <option value="Faqirwali">Faqirwali</option>
                                        <option value="Fort Abbas">Fort Abbas</option>
                                        <option value="Harunabad">Harunabad</option>
                                        <option value="Mandi Sadiqganj">Mandi Sadiqganj</option>
                                        <option value="Minchinabad">Minchinabad</option>
                                        <option value="Ahmadpur East16">Ahmadpur East</option>
                                        <option value="Bahawalpur">Bahawalpur</option>
                                        <option value="Hasilpur">Hasilpur</option>
                                        <option value="1Khairpur Tamewali9">Khairpur Tamewali</option>
                                        <option value="Samasata">Samasata</option>
                                        <option value="Uch (Uch Sharif)">Uch (Uch Sharif)</option>
                                        <option value="Yazman">Yazman</option>
                                        <option value="Bhakkar">Bhakkar</option>
                                        <option value="Darya Khan">Darya Khan</option>
                                        <option value="Dullewala">Dullewala</option>
                                        <option value="Jandanwala">Jandanwala</option>
                                        <option value="Kalur Kot">Kalur Kot</option>
                                        <option value="Mankera">Mankera</option>
                                        <option value="Bhaun">Bhaun</option>
                                        <option value="Chakwal">Chakwal</option>
                                        <option value="Choa Saidan Shah">Choa Saidan Shah</option>
                                        <option value="Kallar Kahar">Kallar Kahar</option>
                                        <option value="Lawa">Lawa</option>
                                        <option value="Talagang">Talagang</option>
                                        <option value="Bhawana">Bhawana</option>
                                        <option value="Chenab Nagar">Chenab Nagar</option>
                                        <option value="Chiniot">Chiniot</option>
                                        <option value="Lalian">Lalian</option>
                                        <option value="Dera Ghazi Khan">Dera Ghazi Khan</option>
                                        <option value="Kot Chutta">Kot Chutta</option>
                                        <option value="Taunsa">Taunsa</option>
                                        <option value="Chak Jhumra">Chak Jhumra</option>
                                        <option value="Dijkot">Dijkot</option>
                                        <option value="Faisalabad">Faisalabad</option>
                                        <option value="Jaranwala">Jaranwala</option>
                                        <option value="Khurianwala">Khurianwala</option>
                                        <option value="Mamu Kanjan">Mamu Kanjan</option>
                                        <option value="Samundri">Samundri</option>
                                        <option value="Tandlianwala">Tandlianwala</option>
                                        <option value="Alipur Chatha">Alipur Chatha</option>
                                        <option value="Dhaunkal">Dhaunkal</option>
                                        <option value="Eminabad">Eminabad</option>
                                        <option value="Ghakhar">Ghakhar</option>
                                        <option value="Gujranwala">Gujranwala</option>
                                        <option value="Kamoki">Kamoki</option>
                                        <option value="Ludhewala Warraich">Ludhewala Warraich</option>
                                        <option value="Nowshera Virkan">Nowshera Virkan</option>
                                        <option value="Qila Didar Singh">Qila Didar Singh</option>
                                        <option value="Rasulnagar">Rasulnagar</option>
                                        <option value="Sodhra">Sodhra</option>
                                        <option value="Wazirabad">Wazirabad</option>
                                        <option value="Dinga">Dinga</option>
                                        <option value="Gujrat">Gujrat</option>
                                        <option value="Jalalpur Jattan">Jalalpur Jattan</option>
                                        <option value="Kharian">Kharian</option>
                                        <option value="Kunjah">Kunjah</option>
                                        <option value="Lala Musa">Lala Musa</option>
                                        <option value="Sarai Alamgir">Sarai Alamgir</option>
                                        <option value="Shadiwal">Shadiwal</option>
                                        <option value="Hafizabad">Hafizabad</option>
                                        <option value="Jalalpur Bhattian">Jalalpur Bhattian</option>
                                        <option value="Kaleke">Kaleke</option>
                                        <option value="Pindi Bhattian">Pindi Bhattian</option>
                                        <option value="Sukheke">Sukheke</option>
                                        <option value="Athara Hazari">Athara Hazari</option>
                                        <option value="Ahmadpur Sial">Ahmadpur Sial</option>
                                        <option value="Bagh (Punjab)">Bagh (Punjab)</option>
                                        <option value="Garh Maharaja">Garh Maharaja</option>
                                        <option value="Jhang">Jhang</option>
                                        <option value="Shorkot">Shorkot</option>
                                        <option value="Dina">Dina</option>
                                        <option value="Jhelum">Jhelum</option>
                                        <option value="Khewra">Khewra</option>
                                        <option value="Pind Dadan Khan">Pind Dadan Khan</option>
                                        <option value="Sohawa">Sohawa</option>
                                        <option value="Allahabad">Allahabad</option>
                                        <option value="Chunian">Chunian</option>
                                        <option value="Kanganpur">Kanganpur</option>
                                        <option value="Kasur">Kasur</option>
                                        <option value="Khudian">Khudian</option>
                                        <option value="Kot Radha Kishan">Kot Radha Kishan</option>
                                        <option value="Mustafabad">Mustafabad</option>
                                        <option value="Pattoki">Pattoki</option>
                                        <option value="Phulnagar">Phulnagar</option>
                                        <option value="Raja Jang">Raja Jang</option>
                                        <option value="Abdul Hakim">Abdul Hakim</option>
                                        <option value="Jahanian">Jahanian</option>
                                        <option value="Kabirwala">Kabirwala</option>
                                        <option value="Khanewal">Khanewal</option>
                                        <option value="Makhdoom Pur Pahuran">Makhdoom Pur Pahuran</option>
                                        <option value="Mian Channu">Mian Channu</option>
                                        <option value="Sarai Sidhu">Sarai Sidhu</option>
                                        <option value="Tulamba">Tulamba</option>
                                        <option value="Hadali">Hadali</option>
                                        <option value="Jauharabad">Jauharabad</option>
                                        <option value="Khushab">Khushab</option>
                                        <option value="Mitha Tiwana">Mitha Tiwana</option>
                                        <option value="Naushahra">Naushahra</option>
                                        <option value="Noorpur Thal">Noorpur Thal</option>
                                        <option value="Quaidabad">Quaidabad</option>
                                        <option value="Lahore">Lahore</option>
                                        <option value="Chaubara">Chaubara</option>
                                        <option value="Chowk azam">Chowk azam</option>
                                        <option value="Fateh Pur">Fateh Pur</option>
                                        <option value="Karor Lal Esan">Karor Lal Esan</option>
                                        <option value="Layyah">Layyah</option>
                                        <option value="Dhanote">Dhanote</option>
                                        <option value="Dunyapur">Dunyapur</option>
                                        <option value="Kahror Pakka">Kahror Pakka</option>
                                        <option value="Lodhran">Lodhran</option>
                                        <option value="Malakwal">Malakwal</option>
                                        <option value="Mandi Bahauddin">Mandi Bahauddin</option>
                                        <option value="Mangat">Mangat</option>
                                        <option value="Phalia">Phalia</option>
                                        <option value="Qadirabad">Qadirabad</option>
                                        <option value="Daud Khel">Daud Khel</option>
                                        <option value="Harnoli">Harnoli</option>
                                        <option value="Isa Khel">Isa Khel</option>
                                        <option value="Kalabagh">Kalabagh</option>
                                        <option value="Kamar Mashani">Kamar Mashani</option>
                                        <option value="Kundian">Kundian</option>
                                        <option value="Liaquatabad">Liaquatabad</option>
                                        <option value="Mianwali">Mianwali</option>
                                        <option value="Jalalpur Pirwala">Jalalpur Pirwala</option>
                                        <option value="Makhdum Rashid">Makhdum Rashid</option>
                                        <option value="Multan">Multan</option>
                                        <option value="Qadirpur Ran">Qadirpur Ran</option>
                                        <option value="Shujaabad">Shujaabad</option>
                                        <option value="Alipur">Alipur</option>
                                        <option value="Chowk Sarwar Shahid">Chowk Sarwar Shahid</option>
                                        <option value="Dera Din Panah">Dera Din Panah</option>
                                        <option value="Jatoi">Jatoi</option>
                                        <option value="Khangarh (Punjab)">Khangarh (Punjab)</option>
                                        <option value="Kot Addu">Kot Addu</option>
                                        <option value="Muzaffargarh">Muzaffargarh</option>
                                        <option value="Sanawan">Sanawan</option>
                                        <option value="Shehr Sultan">Shehr Sultan</option>
                                        <option value="Nankana Sahib">Nankana Sahib</option>
                                        <option value="Sangla Hill">Sangla Hill</option>
                                        <option value="Shahkot">Shahkot</option>
                                        <option value="Warburton">Warburton</option>
                                        <option value="Baddomalhi">Baddomalhi</option>
                                        <option value="Narowal">Narowal</option>
                                        <option value="Qila Sobha Singh">Qila Sobha Singh</option>
                                        <option value="Shakargarh">Shakargarh</option>
                                        <option value="Zafarwal">Zafarwal</option>
                                        <option value="Ahmadabad">Ahmadabad</option>
                                        <option value="Basirpur">Basirpur</option>
                                        <option value="Dipalpur">Dipalpur</option>
                                        <option value="Gogera">Gogera</option>
                                        <option value="Haveli Lakha">Haveli Lakha</option>
                                        <option value="Hujra Shah Muqim">Hujra Shah Muqim</option>
                                        <option value="Okara">Okara</option>
                                        <option value="Renala Khurd">Renala Khurd</option>
                                        <option value="Arifwala">Arifwala</option>
                                        <option value="Pakpattan">Pakpattan</option>
                                        <option value="Ahmedpur Lumma">Ahmedpur Lumma</option>
                                        <option value="Khanpur (Punjab)">Khanpur (Punjab)</option>
                                        <option value="Kot Samaba">Kot Samaba</option>
                                        <option value="Liaquatpur">Liaquatpur</option>
                                        <option value="Rahimyar Khan">Rahimyar Khan</option>
                                        <option value="Sadiqabad">Sadiqabad</option>
                                        <option value="Trinda Sawai Khan">Trinda Sawai Khan</option>
                                        <option value="Zahir Pir">Zahir Pir</option>
                                        <option value="Dajal">Dajal</option>
                                        <option value="Fazilpur">Fazilpur</option>
                                        <option value="Jampur">Jampur</option>
                                        <option value="Kot Mithan">Kot Mithan</option>
                                        <option value="Rajanpur">Rajanpur</option>
                                        <option value="Rojhan">Rojhan</option>
                                        <option value="Daultala">Daultala</option>
                                        <option value="Gujar Khan">Gujar Khan</option>
                                        <option value="Kahuta">Kahuta</option>
                                        <option value="Kallar Syedan">Kallar Syedan</option>
                                        <option value="Kotli Sattian">Kotli Sattian</option>
                                        <option value="Murree">Murree</option>
                                        <option value="Rawalpindi">Rawalpindi</option>
                                        <option value="Taxila">Taxila</option>
                                        <option value="Chichawatni">Chichawatni</option>
                                        <option value="Kamir">Kamir</option>
                                        <option value="Sahiwal City">Sahiwal City</option>
                                        <option value="Bhalwal">Bhalwal</option>
                                        <option value="Bhera">Bhera</option>
                                        <option value="Farooqa">Farooqa</option>
                                        <option value="Jhawarian">Jhawarian</option>
                                        <option value="Kot Mumin">Kot Mumin</option>
                                        <option value="Lilliani">Lilliani</option>
                                        <option value="Miani">Miani</option>
                                        <option value="Phularwan">Phularwan</option>
                                        <option value="Sahiwal Tehsil">Sahiwal Tehsil</option>
                                        <option value="Sargodha">Sargodha</option>
                                        <option value="Shahpur">Shahpur</option>
                                        <option value="Shahpur Sadar">Shahpur Sadar</option>
                                        <option value="Sillanwali">Sillanwali</option>
                                        <option value="Faruqabad">Faruqabad</option>
                                        <option value="Firozwala">Firozwala</option>
                                        <option value="Jandiala Sher Khan">Jandiala Sher Khan</option>
                                        <option value="Khanqah Dogran">Khanqah Dogran</option>
                                        <option value="Kot Abdul Malik">Kot Abdul Malik</option>
                                        <option value="Mananwala Jodh Singh">Mananwala Jodh Singh</option>
                                        <option value="Muridke">Muridke</option>
                                        <option value="Narang (Narang Mandi)">Narang (Narang Mandi)</option>
                                        <option value="Safdarabad">Safdarabad</option>
                                        <option value="Sharqpur">Sharqpur</option>
                                        <option value="Sheikhupura">Sheikhupura</option>
                                        <option value="Begowala">Begowala</option>
                                        <option value="Bhopalwala">Bhopalwala</option>
                                        <option value="Chawinda">Chawinda</option>
                                        <option value="Daska">Daska</option>
                                        <option value="Jamke Chima">Jamke Chima</option>
                                        <option value="Kalaswala">Kalaswala</option>
                                        <option value="Kotli Loharan">Kotli Loharan</option>
                                        <option value="Pasrur">Pasrur</option>
                                        <option value="Sambrial">Sambrial</option>
                                        <option value="Sialkot">Sialkot</option>
                                        <option value="Gojra">Gojra</option>
                                        <option value="Kamalia">Kamalia</option>
                                        <option value="Pir Mahal">Pir Mahal</option>
                                        <option value="Toba Tek Singh">Toba Tek Singh</option>
                                        <option value="Burewala">Burewala</option>
                                        <option value="Jalla Jeem">Jalla Jeem</option>
                                        <option value="Karampur (Punjab)">Karampur (Punjab)</option>
                                        <option value="Mailsi">Mailsi</option>
                                        <option value="Tibba Sultanpur">Tibba Sultanpur</option>
                                        <option value="Vehari">Vehari</option>
                                        <option value="Awaran">Awaran</option>
                                        <option value="Barkhan">Barkhan</option>
                                        <option value="Bela">Bela</option>
                                        <option value="Bhag">Bhag</option>
                                        <option value="Bulaida (Buleda)">Bulaida (Buleda)</option>
                                        <option value="Chaman">Chaman</option>
                                        <option value="Chitkan">Chitkan</option>
                                        <option value="Dalband?n">Dalband?n</option>
                                        <option value="Dera Allah Yar">Dera Allah Yar</option>
                                        <option value="Dera Bugti">Dera Bugti</option>
                                        <option value="Dera Murad Jamali">Dera Murad Jamali</option>
                                        <option value="Dhadar">Dhadar</option>
                                        <option value="Duki">Duki</option>
                                        <option value="Dureji">Dureji</option>
                                        <option value="Gadani">Gadani</option>
                                        <option value="Gandawah">Gandawah</option>
                                        <option value="Gujar Mashkai">Gujar Mashkai</option>
                                        <option value="Gwadar">Gwadar</option>
                                        <option value="Harnai">Harnai</option>
                                        <option value="Hub">Hub</option>
                                        <option value="Huramzai">Huramzai</option>
                                        <option value="Jiwani">Jiwani</option>
                                        <option value="Kalat">Kalat</option>
                                        <option value="Khanozai">Khanozai</option>
                                        <option value="Kharan">Kharan</option>
                                        <option value="Khuzdar">Khuzdar</option>
                                        <option value="Killa Abdullah">Killa Abdullah</option>
                                        <option value="Killa Saifullah">Killa Saifullah</option>
                                        <option value="Kohlu">Kohlu</option>
                                        <option value="Loralai">Loralai</option>
                                        <option value="Mach (Machh)">Mach (Machh)</option>
                                        <option value="Mastung">Mastung</option>
                                        <option value="Musa Khel (Musakhel Bazar)">Musa Khel (Musakhel Bazar)</option>
                                        <option value="Muslim Bagh">Muslim Bagh</option>
                                        <option value="Nal">Nal</option>
                                        <option value="Nushki">Nushki</option>
                                        <option value="Ormara">Ormara</option>
                                        <option value="Pasni">Pasni</option>
                                        <option value="Pishin">Pishin</option>
                                        <option value="Quetta">Quetta</option>
                                        <option value="Saranan">Saranan</option>
                                        <option value="Shahrig">Shahrig</option>
                                        <option value="Sibi">Sibi</option>
                                        <option value="Sohbatpur">Sohbatpur</option>
                                        <option value="Sui">Sui</option>
                                        <option value="Surab">Surab</option>
                                        <option value="Tasp">Tasp</option>
                                        <option value="Tump">Tump</option>
                                        <option value="Turbat">Turbat</option>
                                        <option value="Usta Muhammad">Usta Muhammad</option>
                                        <option value="Uthal">Uthal</option>
                                        <option value="Wadh">Wadh</option>
                                        <option value="Washuk">Washuk</option>
                                        <option value="Winder (Windar)">Winder (Windar)</option>
                                        <option value="Zehri">Zehri</option>
                                        <option value="Zhob">Zhob</option>
                                        <option value="Ziarat">Ziarat</option>
                                        <option value="Abbottabad">Abbottabad</option>
                                        <option value="Akora Khattak">Akora Khattak</option>
                                        <option value="Amangarh">Amangarh</option>
                                        <option value="Baffa">Baffa</option>
                                        <option value="Bannu">Bannu</option>
                                        <option value="Barikot">Barikot</option>
                                        <option value="Bat Khela">Bat Khela</option>
                                        <option value="Behrain">Behrain</option>
                                        <option value="Charsadda">Charsadda</option>
                                        <option value="Chitral">Chitral</option>
                                        <option value="Dera Ismail Khan">Dera Ismail Khan</option>
                                        <option value="Dir">Dir</option>
                                        <option value="Doaba">Doaba</option>
                                        <option value="Hangu">Hangu</option>
                                        <option value="Haripur">Haripur</option>
                                        <option value="Havelian">Havelian</option>
                                        <option value="Jehangira">Jehangira</option>
                                        <option value="Kabal">Kabal</option>
                                        <option value="Karak">Karak</option>
                                        <option value="Khalabat">Khalabat</option>
                                        <option value="Khwazakhela">Khwazakhela</option>
                                        <option value="Kohat">Kohat</option>
                                        <option value="Kulachi">Kulachi</option>
                                        <option value="Lachi">Lachi</option>
                                        <option value="Lakki Marwat">Lakki Marwat</option>
                                        <option value="Mansehra">Mansehra</option>
                                        <option value="Mardan">Mardan</option>
                                        <option value="Matta">Matta</option>
                                        <option value="Mingawara">Mingawara</option>
                                        <option value="Nawan Killi (Kernal Sher Killi)">Nawan Killi (Kernal Sher Killi)
                                        </option>
                                        <option value="Nawanshehr">Nawanshehr</option>
                                        <option value="Nowshera Kalan">Nowshera Kalan</option>
                                        <option value="Pabbi">Pabbi</option>
                                        <option value="Paharpur">Paharpur</option>
                                        <option value="Paniala">Paniala</option>
                                        <option value="Paroa">Paroa</option>
                                        <option value="Peshawar">Peshawar</option>
                                        <option value="Peshawar University">Peshawar University</option>
                                        <option value="Serai Naurang (Naurang)">Serai Naurang (Naurang)</option>
                                        <option value="Shabqadar">Shabqadar</option>
                                        <option value="Shakardara">Shakardara</option>
                                        <option value="Swabi">Swabi</option>
                                        <option value="Takht Bhai">Takht Bhai</option>
                                        <option value="Tangi">Tangi</option>
                                        <option value="Tank">Tank</option>
                                        <option value="Thall">Thall</option>
                                        <option value="Timargara">Timargara</option>
                                        <option value="Topi">Topi</option>
                                        <option value="Tordher">Tordher</option>
                                        <option value="Utmanzai">Utmanzai</option>
                                        <option value="Zaida">Zaida</option>
                                        <option value="Adalpur">Adalpur</option>
                                        <option value="Agra">Agra</option>
                                        <option value="Ahmedpur">Ahmedpur</option>
                                        <option value="Arija">Arija</option>
                                        <option value="Babarloi">Babarloi</option>
                                        <option value="Badah">Badah</option>
                                        <option value="Badin">Badin</option>
                                        <option value="Bagerji">Bagerji</option>
                                        <option value="Bandhi">Bandhi</option>
                                        <option value="Behram">Behram</option>
                                        <option value="Berani">Berani</option>
                                        <option value="Bhan">Bhan</option>
                                        <option value="Bhiria">Bhiria</option>
                                        <option value="Bhiria Road">Bhiria Road</option>
                                        <option value="Bhit Shah (Bhit)">Bhit Shah (Bhit)</option>
                                        <option value="Bolhari">Bolhari</option>
                                        <option value="Bozdar Wada">Bozdar Wada</option>
                                        <option value="Bucheri">Bucheri</option>
                                        <option value="Bulri Shah Karim">Bulri Shah Karim</option>
                                        <option value="Buxapur (Bakshapur)">Buxapur (Bakshapur)</option>
                                        <option value="Chachro">Chachro</option>
                                        <option value="Chak">Chak</option>
                                        <option value="Chambar">Chambar</option>
                                        <option value="Chelhar">Chelhar</option>
                                        <option value="Choondiko">Choondiko</option>
                                        <option value="Chore Old">Chore Old</option>
                                        <option value="Chuhar Jamali">Chuhar Jamali</option>
                                        <option value="Dadu">Dadu</option>
                                        <option value="Daharki">Daharki</option>
                                        <option value="Daro">Daro</option>
                                        <option value="Darya Khan Marri">Darya Khan Marri</option>
                                        <option value="Daulatpur">Daulatpur</option>
                                        <option value="Daur">Daur</option>
                                        <option value="Dhoronaro">Dhoronaro</option>
                                        <option value="Digri">Digri</option>
                                        <option value="Diplo">Diplo</option>
                                        <option value="Dokri">Dokri</option>
                                        <option value="Faqirabad">Faqirabad</option>
                                        <option value="Gaji Khuhawar">Gaji Khuhawar</option>
                                        <option value="Gambat">Gambat</option>
                                        <option value="Garello">Garello</option>
                                        <option value="Garhi Khairo">Garhi Khairo</option>
                                        <option value="Garhi Yasin">Garhi Yasin</option>
                                        <option value="Gharo">Gharo</option>
                                        <option value="Ghauspur">Ghauspur</option>
                                        <option value="Ghorabari">Ghorabari</option>
                                        <option value="Ghotki">Ghotki</option>
                                        <option value="Guddu">Guddu</option>
                                        <option value="Hala">Hala</option>
                                        <option value="Halani">Halani</option>
                                        <option value="Hala Old">Hala Old</option>
                                        <option value="Hingorja">Hingorja</option>
                                        <option value="Hingorno">Hingorno</option>
                                        <option value="Husri">Husri</option>
                                        <option value="Hyderabad">Hyderabad</option>
                                        <option value="Islamkot">Islamkot</option>

                                        <option value="Jacobabad">Jacobabad</option>
                                        <option value="Jam Nawaz Ali">Jam Nawaz Ali</option>
                                        <option value="Jam Sahib">Jam Sahib</option>
                                        <option value="Jamshoro">Jamshoro</option>
                                        <option value="Jati">Jati</option>
                                        <option value="Jatia">Jatia</option>
                                        <option value="Jhol">Jhol</option>
                                        <option value="Jhudo">Jhudo</option>
                                        <option value="Johi">Johi</option>
                                        <option value="Kadhan">Kadhan</option>
                                        <option value="Kambar Ali Khan (Kambar)">Kambar Ali Khan (Kambar)</option>
                                        <option value="Kandhkot">Kandhkot</option>
                                        <option value="Kandhra">Kandhra</option>
                                        <option value="Kandiari">Kandiari</option>
                                        <option value="Kandiaro">Kandiaro</option>
                                        <option value="Karachi">Karachi</option>
                                        <option value="Karampur (Sindh)">Karampur (Sindh)</option>
                                        <option value="Kario Ghanwer">Kario Ghanwer</option>
                                        <option value="Karundi">Karundi</option>
                                        <option value="Kashmor">Kashmor</option>
                                        <option value="Kazi Ahmed">Kazi Ahmed</option>
                                        <option value="Keti Bunder">Keti Bunder</option>
                                        <option value="Khadro">Khadro</option>
                                        <option value="Khairpur">Khairpur</option>
                                        <option value="Khairpur Nathan Shah">Khairpur Nathan Shah</option>
                                        <option value="Khangarh (Sindh)">Khangarh (Sindh)</option>
                                        <option value="Khanpur (Sindh)">Khanpur (Sindh)</option>
                                        <option value="Kheme-Jo-Par">Kheme-Jo-Par</option>
                                        <option value="Khipro">Khipro</option>
                                        <option value="Khoski">Khoski</option>
                                        <option value="Khuhra">Khuhra</option>
                                        <option value="Khyber">Khyber</option>
                                        <option value="Kot Diji">Kot Diji</option>
                                        <option value="Kot Ghulam Muhammad">Kot Ghulam Muhammad</option>
                                        <option value="Kotri">Kotri</option>
                                        <option value="Kumb">Kumb</option>
                                        <option value="Kunri">Kunri</option>
                                        <option value="Lakhi">Lakhi</option>
                                        <option value="Larkana">Larkana</option>
                                        <option value="Madeji">Madeji</option>
                                        <option value="Makli">Makli</option>
                                        <option value="Manjhand">Manjhand</option>
                                        <option value="Matiari">Matiari</option>
                                        <option value="Matli">Matli</option>
                                        <option value="Mehar">Mehar</option>
                                        <option value="Mehrabpur">Mehrabpur</option>
                                        <option value="Miro Khan">Miro Khan</option>
                                        <option value="Mirpur Bathoro">Mirpur Bathoro</option>
                                        <option value="Mirpur Buriro">Mirpur Buriro</option>
                                        <option value="Mirpur Khas">Mirpur Khas</option>
                                        <option value="Mirpur Mathelo">Mirpur Mathelo</option>
                                        <option value="Mirpur Sakro">Mirpur Sakro</option>
                                        <option value="Mirwah">Mirwah</option>
                                        <option value="Mithi">Mithi</option>
                                        <option value="Mithiani">Mithiani</option>
                                        <option value="Moro">Moro</option>
                                        <option value="Nabisar">Nabisar</option>
                                        <option value="Nagar Parkar">Nagar Parkar</option>
                                        <option value="Nasirabad">Nasirabad</option>
                                        <option value="Nasirpur">Nasirpur</option>
                                        <option value="Naudero">Naudero</option>
                                        <option value="Naukot">Naukot</option>
                                        <option value="Naushahro Feroze">Naushahro Feroze</option>
                                        <option value="Nawabshah">Nawabshah</option>
                                        <option value="New Saeedabad (Saeedabad)">New Saeedabad (Saeedabad)</option>
                                        <option value="Nindo">Nindo</option>
                                        <option value="Odero Lal Station">Odero Lal Station</option>
                                        <option value="Pacca Chang">Pacca Chang</option>
                                        <option value="Padidan">Padidan</option>
                                        <option value="Pangrio">Pangrio</option>
                                        <option value="Pano Aqil">Pano Aqil</option>
                                        <option value="Phulji">Phulji</option>
                                        <option value="Piara Lund">Piara Lund</option>
                                        <option value="Pirjo Goth (Kingri)">Pirjo Goth (Kingri)</option>
                                        <option value="Piru Mal">Piru Mal</option>
                                        <option value="Piryaloi">Piryaloi</option>
                                        <option value="Pithoro">Pithoro</option>
                                        <option value="Qasimabad">Qasimabad</option>
                                        <option value="Qubo Saeed Khan">Qubo Saeed Khan</option>
                                        <option value="Radhan">Radhan</option>
                                        <option value="Rajo Khanani">Rajo Khanani</option>
                                        <option value="Ranipur">Ranipur</option>
                                        <option value="Ratodero">Ratodero</option>
                                        <option value="Rohri">Rohri</option>
                                        <option value="Rustam">Rustam</option>
                                        <option value="Sakrand">Sakrand</option>
                                        <option value="Salehpat">Salehpat</option>
                                        <option value="Samaro">Samaro</option>
                                        <option value="Sanghar">Sanghar</option>
                                        <option value="Sanjar Chang">Sanjar Chang</option>
                                        <option value="Sann">Sann</option>
                                        <option value="Sarhari">Sarhari</option>
                                        <option value="Sehwan (Sehwan Sharif)">Sehwan (Sehwan Sharif)</option>
                                        <option value="Setharja">Setharja</option>
                                        <option value="Shadipali">Shadipali</option>
                                        <option value="Shahdadkot">Shahdadkot</option>
                                        <option value="Shahdadpur">Shahdadpur</option>
                                        <option value="Shaheed Fazal Rahu (Golarchi)">Shaheed Fazal Rahu (Golarchi)
                                        </option>
                                        <option value="Shahpur Chakar">Shahpur Chakar</option>
                                        <option value="Shahpur Jahania">Shahpur Jahania</option>

                                        <option value="Shikarpur">Shikarpur</option>
                                        <option value="Sinjhoro">Sinjhoro</option>
                                        <option value="Sita Road">Sita Road</option>
                                        <option value="Sobho Dero">Sobho Dero</option>
                                        <option value="Suhrab Khan Sarki">Suhrab Khan Sarki</option>
                                        <option value="Sujawal">Sujawal</option>
                                        <option value="Sukkur">Sukkur</option>
                                        <option value="Sultanabad">Sultanabad</option>
                                        <option value="Talhar">Talhar</option>
                                        <option value="Tando Adam">Tando Adam</option>
                                        <option value="Tando Allahyar">Tando Allahyar</option>
                                        <option value="Tando Bago">Tando Bago</option>
                                        <option value="Tando Ghulam Ali">Tando Ghulam Ali</option>
                                        <option value="Tando Ghulam Hyder">Tando Ghulam Hyder</option>
                                        <option value="Tando Jam">Tando Jam</option>
                                        <option value="Tando Jan Muhammad">Tando Jan Muhammad</option>
                                        <option value="Tando Mir Ali">Tando Mir Ali</option>
                                        <option value="Tando Mitha Khan">Tando Mitha Khan</option>
                                        <option value="Tando Muhammad Khan">Tando Muhammad Khan</option>
                                        <option value="Tangwani">Tangwani</option>
                                        <option value="Thano Bula Khan">Thano Bula Khan</option>
                                        <option value="Thari Mirwah">Thari Mirwah</option>
                                        <option value="Thariri Muhabbat">Thariri Muhabbat</option>
                                        <option value="Tharushah">Tharushah</option>
                                        <option value="Thatta">Thatta</option>
                                        <option value="Therhi (Therhi I)">Therhi (Therhi I)</option>
                                        <option value="Therhi II">Therhi II</option>
                                        <option value="Thul">Thul</option>
                                        <option value="Ubauro">Ubauro</option>
                                        <option value="Umerkot">Umerkot</option>
                                        <option value="Wagan">Wagan</option>
                                        <option value="Warah">Warah</option>
                                        <option value="Khaplu">Khaplu</option>
                                        <option value="Skardu">Skardu</option>
                                        <option value="Astore">Astore</option>
                                        <option value="Diamer">Diamer</option>
                                        <option value="Ghizer">Ghizer</option>
                                        <option value="Gilgit">Gilgit</option>
                                        <option value="Hunza">Hunza</option>
                                        <option value="Kharmang">Kharmang</option>
                                        <option value="Shigar">Shigar</option>
                                        <option value="Muzaffarabad">Muzaffarabad</option>
                                        <option value="Jhelum valley">Jhelum valley</option>
                                        <option value="Neelum">Neelum</option>
                                        <option value="Mirpur">Mirpur</option>
                                        <option value="Bhimber">Bhimber</option>
                                        <option value="Kotli">Kotli</option>
                                        <option value="Poonch">Poonch</option>
                                        <option value="Bagh (AJK)">Bagh (AJK)</option>
                                        <option value="Haveli">Haveli</option>
                                        <option value="Sudhnati">Sudhnati</option>
                                        <option value="Bajaur Agency">Bajaur Agency</option>
                                        <option value="Khyber Agency">Khyber Agency</option>
                                        <option value="Kurram Agency">Kurram Agency</option>
                                        <option value="Mohmand Agency">Mohmand Agency</option>
                                        <option value="North Waziristan Agency">North Waziristan Agency</option>
                                        <option value="Orakzai Agency">Orakzai Agency</option>
                                        <option value="South Waziristan Agency">South Waziristan Agency</option>
                                        <option value="FR Bannu">FR Bannu</option>
                                        <option value="FR Dera Ismail Khan">FR Dera Ismail Khan</option>
                                        <option value="FR Kohat">FR Kohat</option>
                                        <option value="FR Lakki Marwat">FR Lakki Marwat</option>
                                        <option value="FR Peshawar">FR Peshawar</option>
                                        <option value="FR Tank">FR Tank</option>
                                        <option value="Islamabad">Islamabad</option>
                                        <option value="Lasbela (Tehsil)">Lasbela (Tehsil)</option>
                                        <option value="Malakand">Malakand</option>
                                        <option value="Nowshera">Nowshera</option>
                                        <option value="Rawalakot">Rawalakot</option>
                                        <option value="Sharingal">Sharingal</option>
                                        <option value="Charbagh">Charbagh</option>
                                        <option value="Babuzai (Swat)">Babuzai (Swat)</option>
                                        <option value="Wah Cantt.">Wah Cantt.</option>
                                        <option value="Seo">Seo</option>
                                        <option value="Besham">Besham</option>
                                        <option value="Shangla">Shangla</option>
                                        <option value="N/A">N/A</option>

                                    </select>
                                    <x-input-error class="mt-2 @error('temp_city') has-error @enderror"
                                        :messages="$errors->get('temp_city')" />
                                </div>
                            </div>

                            <div class="row mb-3 ">
                                <label for="PostalCityId" class="col-sm-2 col-form-label"> Same as Above: </label>
                                <div class="col-sm-2">
                                    &nbsp;&nbsp;&nbsp;<input class="form-check-input SameAsAbove" name="sameAsAbove"
                                        onchange="myChangeFunction()" type="checkbox" value="" id="myInput3" />

                                </div>
                            </div>
                            <div class="row mb-3 ">
                                <label for="per_postal_address" class="col-sm-2 col-form-label"> Permanent Address: <span
                                        class="text-danger">*</span></label>
                                <div class="col-sm-8">
                                    <textarea class="form-control" name="per_postal_address" id="per_postal_address" value="jbd" required>{{ $addressDetail->per_postal_address }}</textarea>
                                    <x-input-error class="mt-2 @error('per_postal_address') has-error @enderror"
                                        :messages="$errors->get('per_postal_address')" />
                                </div>
                            </div>
                            <div class="row mb-3 ">
                                <label for="per_city" class="col-sm-2 col-form-label"> Permanent City: <span
                                        class="text-danger">*</span></label>
                                <div class="col-sm-8">
                                    <select name="per_city" value="{{ old('per_city') }}" id="per_city"
                                        class="form-control form-select select2" required>
                                        <option value="">Choose Permanent City</option>
                                        <option selected value="{{ $addressDetail->per_city }}">
                                            {{ $addressDetail->per_city }}
                                        </option>
                                        <option value="Attock">Attock</option>
                                        <option value="Fatehjang">Fatehjang</option>
                                        <option value="Ghorgushti">Ghorgushti</option>
                                        <option value="Hasan Abdal">Hasan Abdal</option>
                                        <option value="Hazro">Hazro</option>
                                        <option value="Jand">Jand</option>
                                        <option value="Pindi Gheb">Pindi Gheb</option>
                                        <option value="Bahawalnagar">Bahawalnagar</option>
                                        <option value="Chishtian">Chishtian</option>
                                        <option value="Dunga Bunga">Dunga Bunga</option>
                                        <option value="Faqirwali">Faqirwali</option>
                                        <option value="Fort Abbas">Fort Abbas</option>
                                        <option value="Harunabad">Harunabad</option>
                                        <option value="Mandi Sadiqganj">Mandi Sadiqganj</option>
                                        <option value="Minchinabad">Minchinabad</option>
                                        <option value="Ahmadpur East16">Ahmadpur East</option>
                                        <option value="Bahawalpur">Bahawalpur</option>
                                        <option value="Hasilpur">Hasilpur</option>
                                        <option value="1Khairpur Tamewali9">Khairpur Tamewali</option>
                                        <option value="Samasata">Samasata</option>
                                        <option value="Uch (Uch Sharif)">Uch (Uch Sharif)</option>
                                        <option value="Yazman">Yazman</option>
                                        <option value="Bhakkar">Bhakkar</option>
                                        <option value="Darya Khan">Darya Khan</option>
                                        <option value="Dullewala">Dullewala</option>
                                        <option value="Jandanwala">Jandanwala</option>
                                        <option value="Kalur Kot">Kalur Kot</option>
                                        <option value="Mankera">Mankera</option>
                                        <option value="Bhaun">Bhaun</option>
                                        <option value="Chakwal">Chakwal</option>
                                        <option value="Choa Saidan Shah">Choa Saidan Shah</option>
                                        <option value="Kallar Kahar">Kallar Kahar</option>
                                        <option value="Lawa">Lawa</option>
                                        <option value="Talagang">Talagang</option>
                                        <option value="Bhawana">Bhawana</option>
                                        <option value="Chenab Nagar">Chenab Nagar</option>
                                        <option value="Chiniot">Chiniot</option>
                                        <option value="Lalian">Lalian</option>
                                        <option value="Dera Ghazi Khan">Dera Ghazi Khan</option>
                                        <option value="Kot Chutta">Kot Chutta</option>
                                        <option value="Taunsa">Taunsa</option>
                                        <option value="Chak Jhumra">Chak Jhumra</option>
                                        <option value="Dijkot">Dijkot</option>
                                        <option value="Faisalabad">Faisalabad</option>
                                        <option value="Jaranwala">Jaranwala</option>
                                        <option value="Khurianwala">Khurianwala</option>
                                        <option value="Mamu Kanjan">Mamu Kanjan</option>
                                        <option value="Samundri">Samundri</option>
                                        <option value="Tandlianwala">Tandlianwala</option>
                                        <option value="Alipur Chatha">Alipur Chatha</option>
                                        <option value="Dhaunkal">Dhaunkal</option>
                                        <option value="Eminabad">Eminabad</option>
                                        <option value="Ghakhar">Ghakhar</option>
                                        <option value="Gujranwala">Gujranwala</option>
                                        <option value="Kamoki">Kamoki</option>
                                        <option value="Ludhewala Warraich">Ludhewala Warraich</option>
                                        <option value="Nowshera Virkan">Nowshera Virkan</option>
                                        <option value="Qila Didar Singh">Qila Didar Singh</option>
                                        <option value="Rasulnagar">Rasulnagar</option>
                                        <option value="Sodhra">Sodhra</option>
                                        <option value="Wazirabad">Wazirabad</option>
                                        <option value="Dinga">Dinga</option>
                                        <option value="Gujrat">Gujrat</option>
                                        <option value="Jalalpur Jattan">Jalalpur Jattan</option>
                                        <option value="Kharian">Kharian</option>
                                        <option value="Kunjah">Kunjah</option>
                                        <option value="Lala Musa">Lala Musa</option>
                                        <option value="Sarai Alamgir">Sarai Alamgir</option>
                                        <option value="Shadiwal">Shadiwal</option>
                                        <option value="Hafizabad">Hafizabad</option>
                                        <option value="Jalalpur Bhattian">Jalalpur Bhattian</option>
                                        <option value="Kaleke">Kaleke</option>
                                        <option value="Pindi Bhattian">Pindi Bhattian</option>
                                        <option value="Sukheke">Sukheke</option>
                                        <option value="Athara Hazari">Athara Hazari</option>
                                        <option value="Ahmadpur Sial">Ahmadpur Sial</option>
                                        <option value="Bagh (Punjab)">Bagh (Punjab)</option>
                                        <option value="Garh Maharaja">Garh Maharaja</option>
                                        <option value="Jhang">Jhang</option>
                                        <option value="Shorkot">Shorkot</option>
                                        <option value="Dina">Dina</option>
                                        <option value="Jhelum">Jhelum</option>
                                        <option value="Khewra">Khewra</option>
                                        <option value="Pind Dadan Khan">Pind Dadan Khan</option>
                                        <option value="Sohawa">Sohawa</option>
                                        <option value="Allahabad">Allahabad</option>
                                        <option value="Chunian">Chunian</option>
                                        <option value="Kanganpur">Kanganpur</option>
                                        <option value="Kasur">Kasur</option>
                                        <option value="Khudian">Khudian</option>
                                        <option value="Kot Radha Kishan">Kot Radha Kishan</option>
                                        <option value="Mustafabad">Mustafabad</option>
                                        <option value="Pattoki">Pattoki</option>
                                        <option value="Phulnagar">Phulnagar</option>
                                        <option value="Raja Jang">Raja Jang</option>
                                        <option value="Abdul Hakim">Abdul Hakim</option>
                                        <option value="Jahanian">Jahanian</option>
                                        <option value="Kabirwala">Kabirwala</option>
                                        <option value="Khanewal">Khanewal</option>
                                        <option value="Makhdoom Pur Pahuran">Makhdoom Pur Pahuran</option>
                                        <option value="Mian Channu">Mian Channu</option>
                                        <option value="Sarai Sidhu">Sarai Sidhu</option>
                                        <option value="Tulamba">Tulamba</option>
                                        <option value="Hadali">Hadali</option>
                                        <option value="Jauharabad">Jauharabad</option>
                                        <option value="Khushab">Khushab</option>
                                        <option value="Mitha Tiwana">Mitha Tiwana</option>
                                        <option value="Naushahra">Naushahra</option>
                                        <option value="Noorpur Thal">Noorpur Thal</option>
                                        <option value="Quaidabad">Quaidabad</option>
                                        <option value="Lahore">Lahore</option>
                                        <option value="Chaubara">Chaubara</option>
                                        <option value="Chowk azam">Chowk azam</option>
                                        <option value="Fateh Pur">Fateh Pur</option>
                                        <option value="Karor Lal Esan">Karor Lal Esan</option>
                                        <option value="Layyah">Layyah</option>
                                        <option value="Dhanote">Dhanote</option>
                                        <option value="Dunyapur">Dunyapur</option>
                                        <option value="Kahror Pakka">Kahror Pakka</option>
                                        <option value="Lodhran">Lodhran</option>
                                        <option value="Malakwal">Malakwal</option>
                                        <option value="Mandi Bahauddin">Mandi Bahauddin</option>
                                        <option value="Mangat">Mangat</option>
                                        <option value="Phalia">Phalia</option>
                                        <option value="Qadirabad">Qadirabad</option>
                                        <option value="Daud Khel">Daud Khel</option>
                                        <option value="Harnoli">Harnoli</option>
                                        <option value="Isa Khel">Isa Khel</option>
                                        <option value="Kalabagh">Kalabagh</option>
                                        <option value="Kamar Mashani">Kamar Mashani</option>
                                        <option value="Kundian">Kundian</option>
                                        <option value="Liaquatabad">Liaquatabad</option>
                                        <option value="Mianwali">Mianwali</option>
                                        <option value="Jalalpur Pirwala">Jalalpur Pirwala</option>
                                        <option value="Makhdum Rashid">Makhdum Rashid</option>
                                        <option value="Multan">Multan</option>
                                        <option value="Qadirpur Ran">Qadirpur Ran</option>
                                        <option value="Shujaabad">Shujaabad</option>
                                        <option value="Alipur">Alipur</option>
                                        <option value="Chowk Sarwar Shahid">Chowk Sarwar Shahid</option>
                                        <option value="Dera Din Panah">Dera Din Panah</option>
                                        <option value="Jatoi">Jatoi</option>
                                        <option value="Khangarh (Punjab)">Khangarh (Punjab)</option>
                                        <option value="Kot Addu">Kot Addu</option>
                                        <option value="Muzaffargarh">Muzaffargarh</option>
                                        <option value="Sanawan">Sanawan</option>
                                        <option value="Shehr Sultan">Shehr Sultan</option>
                                        <option value="Nankana Sahib">Nankana Sahib</option>
                                        <option value="Sangla Hill">Sangla Hill</option>
                                        <option value="Shahkot">Shahkot</option>
                                        <option value="Warburton">Warburton</option>
                                        <option value="Baddomalhi">Baddomalhi</option>
                                        <option value="Narowal">Narowal</option>
                                        <option value="Qila Sobha Singh">Qila Sobha Singh</option>
                                        <option value="Shakargarh">Shakargarh</option>
                                        <option value="Zafarwal">Zafarwal</option>
                                        <option value="Ahmadabad">Ahmadabad</option>
                                        <option value="Basirpur">Basirpur</option>
                                        <option value="Dipalpur">Dipalpur</option>
                                        <option value="Gogera">Gogera</option>
                                        <option value="Haveli Lakha">Haveli Lakha</option>
                                        <option value="Hujra Shah Muqim">Hujra Shah Muqim</option>
                                        <option value="Okara">Okara</option>
                                        <option value="Renala Khurd">Renala Khurd</option>
                                        <option value="Arifwala">Arifwala</option>
                                        <option value="Pakpattan">Pakpattan</option>
                                        <option value="Ahmedpur Lumma">Ahmedpur Lumma</option>
                                        <option value="Khanpur (Punjab)">Khanpur (Punjab)</option>
                                        <option value="Kot Samaba">Kot Samaba</option>
                                        <option value="Liaquatpur">Liaquatpur</option>
                                        <option value="Rahimyar Khan">Rahimyar Khan</option>
                                        <option value="Sadiqabad">Sadiqabad</option>
                                        <option value="Trinda Sawai Khan">Trinda Sawai Khan</option>
                                        <option value="Zahir Pir">Zahir Pir</option>
                                        <option value="Dajal">Dajal</option>
                                        <option value="Fazilpur">Fazilpur</option>
                                        <option value="Jampur">Jampur</option>
                                        <option value="Kot Mithan">Kot Mithan</option>
                                        <option value="Rajanpur">Rajanpur</option>
                                        <option value="Rojhan">Rojhan</option>
                                        <option value="Daultala">Daultala</option>
                                        <option value="Gujar Khan">Gujar Khan</option>
                                        <option value="Kahuta">Kahuta</option>
                                        <option value="Kallar Syedan">Kallar Syedan</option>
                                        <option value="Kotli Sattian">Kotli Sattian</option>
                                        <option value="Murree">Murree</option>
                                        <option value="Rawalpindi">Rawalpindi</option>
                                        <option value="Taxila">Taxila</option>
                                        <option value="Chichawatni">Chichawatni</option>
                                        <option value="Kamir">Kamir</option>
                                        <option value="Sahiwal City">Sahiwal City</option>
                                        <option value="Bhalwal">Bhalwal</option>
                                        <option value="Bhera">Bhera</option>
                                        <option value="Farooqa">Farooqa</option>
                                        <option value="Jhawarian">Jhawarian</option>
                                        <option value="Kot Mumin">Kot Mumin</option>
                                        <option value="Lilliani">Lilliani</option>
                                        <option value="Miani">Miani</option>
                                        <option value="Phularwan">Phularwan</option>
                                        <option value="Sahiwal Tehsil">Sahiwal Tehsil</option>
                                        <option value="Sargodha">Sargodha</option>
                                        <option value="Shahpur">Shahpur</option>
                                        <option value="Shahpur Sadar">Shahpur Sadar</option>
                                        <option value="Sillanwali">Sillanwali</option>
                                        <option value="Faruqabad">Faruqabad</option>
                                        <option value="Firozwala">Firozwala</option>
                                        <option value="Jandiala Sher Khan">Jandiala Sher Khan</option>
                                        <option value="Khanqah Dogran">Khanqah Dogran</option>
                                        <option value="Kot Abdul Malik">Kot Abdul Malik</option>
                                        <option value="Mananwala Jodh Singh">Mananwala Jodh Singh</option>
                                        <option value="Muridke">Muridke</option>
                                        <option value="Narang (Narang Mandi)">Narang (Narang Mandi)</option>
                                        <option value="Safdarabad">Safdarabad</option>
                                        <option value="Sharqpur">Sharqpur</option>
                                        <option value="Sheikhupura">Sheikhupura</option>
                                        <option value="Begowala">Begowala</option>
                                        <option value="Bhopalwala">Bhopalwala</option>
                                        <option value="Chawinda">Chawinda</option>
                                        <option value="Daska">Daska</option>
                                        <option value="Jamke Chima">Jamke Chima</option>
                                        <option value="Kalaswala">Kalaswala</option>
                                        <option value="Kotli Loharan">Kotli Loharan</option>
                                        <option value="Pasrur">Pasrur</option>
                                        <option value="Sambrial">Sambrial</option>
                                        <option value="Sialkot">Sialkot</option>
                                        <option value="Gojra">Gojra</option>
                                        <option value="Kamalia">Kamalia</option>
                                        <option value="Pir Mahal">Pir Mahal</option>
                                        <option value="Toba Tek Singh">Toba Tek Singh</option>
                                        <option value="Burewala">Burewala</option>
                                        <option value="Jalla Jeem">Jalla Jeem</option>
                                        <option value="Karampur (Punjab)">Karampur (Punjab)</option>
                                        <option value="Mailsi">Mailsi</option>
                                        <option value="Tibba Sultanpur">Tibba Sultanpur</option>
                                        <option value="Vehari">Vehari</option>
                                        <option value="Awaran">Awaran</option>
                                        <option value="Barkhan">Barkhan</option>
                                        <option value="Bela">Bela</option>
                                        <option value="Bhag">Bhag</option>
                                        <option value="Bulaida (Buleda)">Bulaida (Buleda)</option>
                                        <option value="Chaman">Chaman</option>
                                        <option value="Chitkan">Chitkan</option>
                                        <option value="Dalband?n">Dalband?n</option>
                                        <option value="Dera Allah Yar">Dera Allah Yar</option>
                                        <option value="Dera Bugti">Dera Bugti</option>
                                        <option value="Dera Murad Jamali">Dera Murad Jamali</option>
                                        <option value="Dhadar">Dhadar</option>
                                        <option value="Duki">Duki</option>
                                        <option value="Dureji">Dureji</option>
                                        <option value="Gadani">Gadani</option>
                                        <option value="Gandawah">Gandawah</option>
                                        <option value="Gujar Mashkai">Gujar Mashkai</option>
                                        <option value="Gwadar">Gwadar</option>
                                        <option value="Harnai">Harnai</option>
                                        <option value="Hub">Hub</option>
                                        <option value="Huramzai">Huramzai</option>
                                        <option value="Jiwani">Jiwani</option>
                                        <option value="Kalat">Kalat</option>
                                        <option value="Khanozai">Khanozai</option>
                                        <option value="Kharan">Kharan</option>
                                        <option value="Khuzdar">Khuzdar</option>
                                        <option value="Killa Abdullah">Killa Abdullah</option>
                                        <option value="Killa Saifullah">Killa Saifullah</option>
                                        <option value="Kohlu">Kohlu</option>
                                        <option value="Loralai">Loralai</option>
                                        <option value="Mach (Machh)">Mach (Machh)</option>
                                        <option value="Mastung">Mastung</option>
                                        <option value="Musa Khel (Musakhel Bazar)">Musa Khel (Musakhel Bazar)</option>
                                        <option value="Muslim Bagh">Muslim Bagh</option>
                                        <option value="Nal">Nal</option>
                                        <option value="Nushki">Nushki</option>
                                        <option value="Ormara">Ormara</option>
                                        <option value="Pasni">Pasni</option>
                                        <option value="Pishin">Pishin</option>
                                        <option value="Quetta">Quetta</option>
                                        <option value="Saranan">Saranan</option>
                                        <option value="Shahrig">Shahrig</option>
                                        <option value="Sibi">Sibi</option>
                                        <option value="Sohbatpur">Sohbatpur</option>
                                        <option value="Sui">Sui</option>
                                        <option value="Surab">Surab</option>
                                        <option value="Tasp">Tasp</option>
                                        <option value="Tump">Tump</option>
                                        <option value="Turbat">Turbat</option>
                                        <option value="Usta Muhammad">Usta Muhammad</option>
                                        <option value="Uthal">Uthal</option>
                                        <option value="Wadh">Wadh</option>
                                        <option value="Washuk">Washuk</option>
                                        <option value="Winder (Windar)">Winder (Windar)</option>
                                        <option value="Zehri">Zehri</option>
                                        <option value="Zhob">Zhob</option>
                                        <option value="Ziarat">Ziarat</option>
                                        <option value="Abbottabad">Abbottabad</option>
                                        <option value="Akora Khattak">Akora Khattak</option>
                                        <option value="Amangarh">Amangarh</option>
                                        <option value="Baffa">Baffa</option>
                                        <option value="Bannu">Bannu</option>
                                        <option value="Barikot">Barikot</option>
                                        <option value="Bat Khela">Bat Khela</option>
                                        <option value="Behrain">Behrain</option>
                                        <option value="Charsadda">Charsadda</option>
                                        <option value="Chitral">Chitral</option>
                                        <option value="Dera Ismail Khan">Dera Ismail Khan</option>
                                        <option value="Dir">Dir</option>
                                        <option value="Doaba">Doaba</option>
                                        <option value="Hangu">Hangu</option>
                                        <option value="Haripur">Haripur</option>
                                        <option value="Havelian">Havelian</option>
                                        <option value="Jehangira">Jehangira</option>
                                        <option value="Kabal">Kabal</option>
                                        <option value="Karak">Karak</option>
                                        <option value="Khalabat">Khalabat</option>
                                        <option value="Khwazakhela">Khwazakhela</option>
                                        <option value="Kohat">Kohat</option>
                                        <option value="Kulachi">Kulachi</option>
                                        <option value="Lachi">Lachi</option>
                                        <option value="Lakki Marwat">Lakki Marwat</option>
                                        <option value="Mansehra">Mansehra</option>
                                        <option value="Mardan">Mardan</option>
                                        <option value="Matta">Matta</option>
                                        <option value="Mingawara">Mingawara</option>
                                        <option value="Nawan Killi (Kernal Sher Killi)">Nawan Killi (Kernal Sher Killi)
                                        </option>
                                        <option value="Nawanshehr">Nawanshehr</option>
                                        <option value="Nowshera Kalan">Nowshera Kalan</option>
                                        <option value="Pabbi">Pabbi</option>
                                        <option value="Paharpur">Paharpur</option>
                                        <option value="Paniala">Paniala</option>
                                        <option value="Paroa">Paroa</option>
                                        <option value="Peshawar">Peshawar</option>
                                        <option value="Peshawar University">Peshawar University</option>
                                        <option value="Serai Naurang (Naurang)">Serai Naurang (Naurang)</option>
                                        <option value="Shabqadar">Shabqadar</option>
                                        <option value="Shakardara">Shakardara</option>
                                        <option value="Swabi">Swabi</option>
                                        <option value="Takht Bhai">Takht Bhai</option>
                                        <option value="Tangi">Tangi</option>
                                        <option value="Tank">Tank</option>
                                        <option value="Thall">Thall</option>
                                        <option value="Timargara">Timargara</option>
                                        <option value="Topi">Topi</option>
                                        <option value="Tordher">Tordher</option>
                                        <option value="Utmanzai">Utmanzai</option>
                                        <option value="Zaida">Zaida</option>
                                        <option value="Adalpur">Adalpur</option>
                                        <option value="Agra">Agra</option>
                                        <option value="Ahmedpur">Ahmedpur</option>
                                        <option value="Arija">Arija</option>
                                        <option value="Babarloi">Babarloi</option>
                                        <option value="Badah">Badah</option>
                                        <option value="Badin">Badin</option>
                                        <option value="Bagerji">Bagerji</option>
                                        <option value="Bandhi">Bandhi</option>
                                        <option value="Behram">Behram</option>
                                        <option value="Berani">Berani</option>
                                        <option value="Bhan">Bhan</option>
                                        <option value="Bhiria">Bhiria</option>
                                        <option value="Bhiria Road">Bhiria Road</option>
                                        <option value="Bhit Shah (Bhit)">Bhit Shah (Bhit)</option>
                                        <option value="Bolhari">Bolhari</option>
                                        <option value="Bozdar Wada">Bozdar Wada</option>
                                        <option value="Bucheri">Bucheri</option>
                                        <option value="Bulri Shah Karim">Bulri Shah Karim</option>
                                        <option value="Buxapur (Bakshapur)">Buxapur (Bakshapur)</option>
                                        <option value="Chachro">Chachro</option>
                                        <option value="Chak">Chak</option>
                                        <option value="Chambar">Chambar</option>
                                        <option value="Chelhar">Chelhar</option>
                                        <option value="Choondiko">Choondiko</option>
                                        <option value="Chore Old">Chore Old</option>
                                        <option value="Chuhar Jamali">Chuhar Jamali</option>
                                        <option value="Dadu">Dadu</option>
                                        <option value="Daharki">Daharki</option>
                                        <option value="Daro">Daro</option>
                                        <option value="Darya Khan Marri">Darya Khan Marri</option>
                                        <option value="Daulatpur">Daulatpur</option>
                                        <option value="Daur">Daur</option>
                                        <option value="Dhoronaro">Dhoronaro</option>
                                        <option value="Digri">Digri</option>
                                        <option value="Diplo">Diplo</option>
                                        <option value="Dokri">Dokri</option>
                                        <option value="Faqirabad">Faqirabad</option>
                                        <option value="Gaji Khuhawar">Gaji Khuhawar</option>
                                        <option value="Gambat">Gambat</option>
                                        <option value="Garello">Garello</option>
                                        <option value="Garhi Khairo">Garhi Khairo</option>
                                        <option value="Garhi Yasin">Garhi Yasin</option>
                                        <option value="Gharo">Gharo</option>
                                        <option value="Ghauspur">Ghauspur</option>
                                        <option value="Ghorabari">Ghorabari</option>
                                        <option value="Ghotki">Ghotki</option>
                                        <option value="Guddu">Guddu</option>
                                        <option value="Hala">Hala</option>
                                        <option value="Halani">Halani</option>
                                        <option value="Hala Old">Hala Old</option>
                                        <option value="Hingorja">Hingorja</option>
                                        <option value="Hingorno">Hingorno</option>
                                        <option value="Husri">Husri</option>
                                        <option value="Hyderabad">Hyderabad</option>
                                        <option value="Islamkot">Islamkot</option>

                                        <option value="Jacobabad">Jacobabad</option>
                                        <option value="Jam Nawaz Ali">Jam Nawaz Ali</option>
                                        <option value="Jam Sahib">Jam Sahib</option>
                                        <option value="Jamshoro">Jamshoro</option>
                                        <option value="Jati">Jati</option>
                                        <option value="Jatia">Jatia</option>
                                        <option value="Jhol">Jhol</option>
                                        <option value="Jhudo">Jhudo</option>
                                        <option value="Johi">Johi</option>
                                        <option value="Kadhan">Kadhan</option>
                                        <option value="Kambar Ali Khan (Kambar)">Kambar Ali Khan (Kambar)</option>
                                        <option value="Kandhkot">Kandhkot</option>
                                        <option value="Kandhra">Kandhra</option>
                                        <option value="Kandiari">Kandiari</option>
                                        <option value="Kandiaro">Kandiaro</option>
                                        <option value="Karachi">Karachi</option>
                                        <option value="Karampur (Sindh)">Karampur (Sindh)</option>
                                        <option value="Kario Ghanwer">Kario Ghanwer</option>
                                        <option value="Karundi">Karundi</option>
                                        <option value="Kashmor">Kashmor</option>
                                        <option value="Kazi Ahmed">Kazi Ahmed</option>
                                        <option value="Keti Bunder">Keti Bunder</option>
                                        <option value="Khadro">Khadro</option>
                                        <option value="Khairpur">Khairpur</option>
                                        <option value="Khairpur Nathan Shah">Khairpur Nathan Shah</option>
                                        <option value="Khangarh (Sindh)">Khangarh (Sindh)</option>
                                        <option value="Khanpur (Sindh)">Khanpur (Sindh)</option>
                                        <option value="Kheme-Jo-Par">Kheme-Jo-Par</option>
                                        <option value="Khipro">Khipro</option>
                                        <option value="Khoski">Khoski</option>
                                        <option value="Khuhra">Khuhra</option>
                                        <option value="Khyber">Khyber</option>
                                        <option value="Kot Diji">Kot Diji</option>
                                        <option value="Kot Ghulam Muhammad">Kot Ghulam Muhammad</option>
                                        <option value="Kotri">Kotri</option>
                                        <option value="Kumb">Kumb</option>
                                        <option value="Kunri">Kunri</option>
                                        <option value="Lakhi">Lakhi</option>
                                        <option value="Larkana">Larkana</option>
                                        <option value="Madeji">Madeji</option>
                                        <option value="Makli">Makli</option>
                                        <option value="Manjhand">Manjhand</option>
                                        <option value="Matiari">Matiari</option>
                                        <option value="Matli">Matli</option>
                                        <option value="Mehar">Mehar</option>
                                        <option value="Mehrabpur">Mehrabpur</option>
                                        <option value="Miro Khan">Miro Khan</option>
                                        <option value="Mirpur Bathoro">Mirpur Bathoro</option>
                                        <option value="Mirpur Buriro">Mirpur Buriro</option>
                                        <option value="Mirpur Khas">Mirpur Khas</option>
                                        <option value="Mirpur Mathelo">Mirpur Mathelo</option>
                                        <option value="Mirpur Sakro">Mirpur Sakro</option>
                                        <option value="Mirwah">Mirwah</option>
                                        <option value="Mithi">Mithi</option>
                                        <option value="Mithiani">Mithiani</option>
                                        <option value="Moro">Moro</option>
                                        <option value="Nabisar">Nabisar</option>
                                        <option value="Nagar Parkar">Nagar Parkar</option>
                                        <option value="Nasirabad">Nasirabad</option>
                                        <option value="Nasirpur">Nasirpur</option>
                                        <option value="Naudero">Naudero</option>
                                        <option value="Naukot">Naukot</option>
                                        <option value="Naushahro Feroze">Naushahro Feroze</option>
                                        <option value="Nawabshah">Nawabshah</option>
                                        <option value="New Saeedabad (Saeedabad)">New Saeedabad (Saeedabad)</option>
                                        <option value="Nindo">Nindo</option>
                                        <option value="Odero Lal Station">Odero Lal Station</option>
                                        <option value="Pacca Chang">Pacca Chang</option>
                                        <option value="Padidan">Padidan</option>
                                        <option value="Pangrio">Pangrio</option>
                                        <option value="Pano Aqil">Pano Aqil</option>
                                        <option value="Phulji">Phulji</option>
                                        <option value="Piara Lund">Piara Lund</option>
                                        <option value="Pirjo Goth (Kingri)">Pirjo Goth (Kingri)</option>
                                        <option value="Piru Mal">Piru Mal</option>
                                        <option value="Piryaloi">Piryaloi</option>
                                        <option value="Pithoro">Pithoro</option>
                                        <option value="Qasimabad">Qasimabad</option>
                                        <option value="Qubo Saeed Khan">Qubo Saeed Khan</option>
                                        <option value="Radhan">Radhan</option>
                                        <option value="Rajo Khanani">Rajo Khanani</option>
                                        <option value="Ranipur">Ranipur</option>
                                        <option value="Ratodero">Ratodero</option>
                                        <option value="Rohri">Rohri</option>
                                        <option value="Rustam">Rustam</option>
                                        <option value="Sakrand">Sakrand</option>
                                        <option value="Salehpat">Salehpat</option>
                                        <option value="Samaro">Samaro</option>
                                        <option value="Sanghar">Sanghar</option>
                                        <option value="Sanjar Chang">Sanjar Chang</option>
                                        <option value="Sann">Sann</option>
                                        <option value="Sarhari">Sarhari</option>
                                        <option value="Sehwan (Sehwan Sharif)">Sehwan (Sehwan Sharif)</option>
                                        <option value="Setharja">Setharja</option>
                                        <option value="Shadipali">Shadipali</option>
                                        <option value="Shahdadkot">Shahdadkot</option>
                                        <option value="Shahdadpur">Shahdadpur</option>
                                        <option value="Shaheed Fazal Rahu (Golarchi)">Shaheed Fazal Rahu (Golarchi)
                                        </option>
                                        <option value="Shahpur Chakar">Shahpur Chakar</option>
                                        <option value="Shahpur Jahania">Shahpur Jahania</option>

                                        <option value="Shikarpur">Shikarpur</option>
                                        <option value="Sinjhoro">Sinjhoro</option>
                                        <option value="Sita Road">Sita Road</option>
                                        <option value="Sobho Dero">Sobho Dero</option>
                                        <option value="Suhrab Khan Sarki">Suhrab Khan Sarki</option>
                                        <option value="Sujawal">Sujawal</option>
                                        <option value="Sukkur">Sukkur</option>
                                        <option value="Sultanabad">Sultanabad</option>
                                        <option value="Talhar">Talhar</option>
                                        <option value="Tando Adam">Tando Adam</option>
                                        <option value="Tando Allahyar">Tando Allahyar</option>
                                        <option value="Tando Bago">Tando Bago</option>
                                        <option value="Tando Ghulam Ali">Tando Ghulam Ali</option>
                                        <option value="Tando Ghulam Hyder">Tando Ghulam Hyder</option>
                                        <option value="Tando Jam">Tando Jam</option>
                                        <option value="Tando Jan Muhammad">Tando Jan Muhammad</option>
                                        <option value="Tando Mir Ali">Tando Mir Ali</option>
                                        <option value="Tando Mitha Khan">Tando Mitha Khan</option>
                                        <option value="Tando Muhammad Khan">Tando Muhammad Khan</option>
                                        <option value="Tangwani">Tangwani</option>
                                        <option value="Thano Bula Khan">Thano Bula Khan</option>
                                        <option value="Thari Mirwah">Thari Mirwah</option>
                                        <option value="Thariri Muhabbat">Thariri Muhabbat</option>
                                        <option value="Tharushah">Tharushah</option>
                                        <option value="Thatta">Thatta</option>
                                        <option value="Therhi (Therhi I)">Therhi (Therhi I)</option>
                                        <option value="Therhi II">Therhi II</option>
                                        <option value="Thul">Thul</option>
                                        <option value="Ubauro">Ubauro</option>
                                        <option value="Umerkot">Umerkot</option>
                                        <option value="Wagan">Wagan</option>
                                        <option value="Warah">Warah</option>
                                        <option value="Khaplu">Khaplu</option>
                                        <option value="Skardu">Skardu</option>
                                        <option value="Astore">Astore</option>
                                        <option value="Diamer">Diamer</option>
                                        <option value="Ghizer">Ghizer</option>
                                        <option value="Gilgit">Gilgit</option>
                                        <option value="Hunza">Hunza</option>
                                        <option value="Kharmang">Kharmang</option>
                                        <option value="Shigar">Shigar</option>
                                        <option value="Muzaffarabad">Muzaffarabad</option>
                                        <option value="Jhelum valley">Jhelum valley</option>
                                        <option value="Neelum">Neelum</option>
                                        <option value="Mirpur">Mirpur</option>
                                        <option value="Bhimber">Bhimber</option>
                                        <option value="Kotli">Kotli</option>
                                        <option value="Poonch">Poonch</option>
                                        <option value="Bagh (AJK)">Bagh (AJK)</option>
                                        <option value="Haveli">Haveli</option>
                                        <option value="Sudhnati">Sudhnati</option>
                                        <option value="Bajaur Agency">Bajaur Agency</option>
                                        <option value="Khyber Agency">Khyber Agency</option>
                                        <option value="Kurram Agency">Kurram Agency</option>
                                        <option value="Mohmand Agency">Mohmand Agency</option>
                                        <option value="North Waziristan Agency">North Waziristan Agency</option>
                                        <option value="Orakzai Agency">Orakzai Agency</option>
                                        <option value="South Waziristan Agency">South Waziristan Agency</option>
                                        <option value="FR Bannu">FR Bannu</option>
                                        <option value="FR Dera Ismail Khan">FR Dera Ismail Khan</option>
                                        <option value="FR Kohat">FR Kohat</option>
                                        <option value="FR Lakki Marwat">FR Lakki Marwat</option>
                                        <option value="FR Peshawar">FR Peshawar</option>
                                        <option value="FR Tank">FR Tank</option>
                                        <option value="Islamabad">Islamabad</option>
                                        <option value="Lasbela (Tehsil)">Lasbela (Tehsil)</option>
                                        <option value="Malakand">Malakand</option>
                                        <option value="Nowshera">Nowshera</option>
                                        <option value="Rawalakot">Rawalakot</option>
                                        <option value="Sharingal">Sharingal</option>
                                        <option value="Charbagh">Charbagh</option>
                                        <option value="Babuzai (Swat)">Babuzai (Swat)</option>
                                        <option value="Wah Cantt.">Wah Cantt.</option>
                                        <option value="Seo">Seo</option>
                                        <option value="Besham">Besham</option>
                                        <option value="Shangla">Shangla</option>
                                        <option value="N/A">N/A</option>
                                    </select>
                                    <x-input-error class="mt-2 @error('per_city') has-error @enderror"
                                        :messages="$errors->get('per_city')" />
                                </div>
                            </div>
                            <div class="row mb-3">
                                <div class="col-sm-10 text-right">
                                    <button type="submit" class="btn btn-primary">Save <span
                                            class="fa fa-save"></span></button>
                                </div>
                            </div>
                        </form>
                    </div>
                </div>
            </div>
        </div>
    </div>
    <script type="text/javascript">
        function myChangeFunction() {
            var postal_address = $('#temp_postal_address').val();
            var postal_city = $('#temp_city').val();
            if ($('#myInput3').is(":checked")) {
                $('#per_postal_address').val(postal_address);
                $('#per_city').val(postal_city).change();
            } else {
                $('#per_postal_address').val(" ");
                $('#per_city').val(" ").change();
            }
        }
    </script>

@endsection
